...
이제 더 이상 제품별 percentage 펌웨어로 보지 않는다.
...
- 일반 회전 액추에이터
- 제한된 각도 범위를 갖는 메커니즘
- 특정 범위만 움직이는 end-effector 시스템

모두에 사용할 수 있다.

제한된 가동범위를 쓰고 싶다면 해당 범위를 `output_min_deg`, `output_max_deg`로 맞춰 쓰면 된다.
...
- 이 펌웨어는 부팅 중 `ActuatorConfig`를 읽을 때, FRAM에 저장된 `can_node_id`가
  현재 빌드 env의 `BUILD_CAN_NODE_ID`와 다르면 FRAM 값을 env 값으로 덮어쓴다.
- 즉 현재 업로드한 PlatformIO env의 node id가 최종적으로 우선한다.
- 여러 보드에 서로 다른 ID를 쓰려면 보드별로 다른 PlatformIO env로 빌드/업로드하면 된다.
- 현재 운영 표준은 steering `S01..S04 -> 1..4`, driving `D01..D04 -> 5..8`이다.
- 반복 배포 절차는 [can_node_id_provisioning.md](/home/gyungminnoh/projects/NoFW/NoFW/docs/can_node_id_provisioning.md)를 따른다.

### 6.5 Calibration 저장 정책

- `ActuatorConfig`는 현재 store version만 읽는다.
  예전 config layout은 migration하지 않고, 읽기 실패 시 현재 firmware default를 새로 저장한다.
- calibration bundle은 FRAM의 두 trusted slot에 저장된다.
- 각 slot은 `magic`, `version`, `sequence`, `crc32`, commit marker, payload를 가진다.
- commit marker는 payload write가 끝난 뒤 마지막에 기록한다.
- 부팅 시 firmware는 header, commit marker, CRC, payload sanity check가 모두 맞는 최신 slot만 읽는다.
- 예전 단일 calibration record는 더 이상 fallback으로 읽지 않는다.

따라서 이 정책이 적용된 펌웨어를 처음 올린 보드는 기존 FRAM에 남아 있던 예전 calibration을 사용하지 않는다.
보드별 첫 `arm`에서 FOC와 출력축 기준을 새로 만들고 trusted slot에 저장해야 한다.
...
- trusted FOC/output calibration 여부
- runtime fault
...
- payload:
  - `data[0..3] = voltage_limit` as `int32 mV`

기본 node `7`에서는 `0x447`이다.
...
- payload:
  - `data[0..3] = voltage_limit` as `int32 mV`
- 예:
  - `30.000 V` -> `cansend can0 2A7#30750000`

저장 조건:

- disarmed 상태에서만 적용된다
- 저장 후 `0x440 + node_id` status에서 확인한다
...
새 trusted calibration slot이 아직 없는 보드는 첫 arm에서 FOC/출력축 기준을 새로 저장한다.
기존 예전 단일 calibration record는 읽지 않는다.
...
- `runtime_fault != 0`이면 먼저 원인을 확인하고 `disarm`으로 fault를 clear한다
