...
- angle command는 현재 저장된 `output_min_deg .. output_max_deg` 범위로 clamp된다
- 따라서 매우 큰 양수/음수 angle command를 보내더라도 내부 목표는 저장된 travel 범위를 넘지 않는다
...
4. 필요하면 `0x227`로 원하는 profile을 바꾼다
5. 먼저 `0x237#01`로 arm 한다
6. 작은 angle command `0x207` 또는 작은 velocity command `0x217`으로 시험한다
7. 시험이 끝나면 `0x237#00`으로 disarm 한다
8. `TMAG`를 쓸 계획이면 calibration runner를 먼저 실행한다

## 14. 현재 벤치에서 확인된 동작 요약
